...
var SideBand = require('./side-band');
...
function Service(opts, req, res) {
	...
	self.once('accept', function() {
		process.nextTick(function() {
			...
			ps.stdout.pipe(respStream, {
				end : !self.hasSideBand
			}).pipe(res);

			if (self.hasSideBand) {
				ps.stdout.once('end', function() {
					var sideBand = new SideBand(respStream);
					self.emit('sideband', sideBand);
				});
			}
...
Service.prototype.sideband = function() {
	this.hasSideBand = true;
	return this;
};
